...
const CORRECTED: Record<string, { value: string; why: string }> = {
  ...
  "m3.units.m3-s2-03.goal": {
    ...
    why: "원문은 '대푯값·산포도·상관관계로 자료의 중심과 퍼짐을 해석합니다.'였다. 대푯값은 한 자료의 중심, 산포도는 그 자료의 퍼짐을 나타내지만 상관관계는 두 변량이 함께 어떻게 변하는지를 나타내는 것이라, 셋을 한 줄에 묶으면 상관관계로 중심·퍼짐을 본다는 뜻이 된다. 정의상 성립하지 않는 문장이라 개념 묶음을 갈랐다. 2026-08-13 수학 내용 검수에서 발견했다.",
  },
  ...
  "m2.units.m2-s2-03.gate.signal": {
    value: "12이면 6×2까지만 계산하고 4로 나누지 않았습니다.",
    why: "원문은 '12이면 비례식의 대응 순서가 무너졌습니다.'였다. 4:6=2:x에서 외항의 곱과 내항의 곱이 같으므로 4x=6×2=12, x=3이다. 대응 순서를 바꾸면 4/3이 나오지 12가 나오지 않는다. 12는 내항의 곱 6×2에서 멈추고 4로 나누지 않은 값이다. 2026-08-13 검수에서 처음 고칠 때 '4×6에서 2로 나누지 않았다'로 잘못 적었다가(그러면 24다) sol 검토에서 다시 잡아 바로잡았다.",
  },
